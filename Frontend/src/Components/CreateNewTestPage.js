...
import { FaBars, FaSun, FaMoon, FaBell, FaSearch, FaArrowLeft } from 'react-icons/fa';
import user from '../Images/user.png';
import CreateMCQQuestions from './CreateMCQQuestions'; // Import your MCQ component
...
const CreateNewTestPage = () => {
  ...
  const renderSection = () => {
    if (selectedSection === 'MCQ') {
      ...
      return (
        <div>
          {/* Your Coding section content here */}
        ...
        <div className="mt-4 p-4 border rounded-md">
          ...
          <div className="space-y-4">
            <button
              ...
              className={`px-6 py-2 ${selectedSection === 'MCQ' ? 'bg-blue-500' : 'bg-gray-500'} text-white rounded-md mr-4`}
            >
              MCQ
            </button>
            <button
              onClick={() => setSelectedSection('Coding')}
              className={`px-6 py-2 ${selectedSection === 'Coding' ? 'bg-blue-500' : 'bg-gray-500'} text-white rounded-md`}
            >
              Coding
            </button>
          </div>
